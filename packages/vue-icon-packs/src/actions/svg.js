...
module.exports = class Component {
  ...
  init() {
    ...
    if (this._outputFiletype === 'sfc')
      jetpack.remove(`tmp/components/${this._library.id}`);
    else jetpack.remove(`tmp/svg/${this._library.id}`);
  ...
  _saveFile(path, svgData) {
    ...
    if (this._outputFiletype === 'sfc') {
      let tmplate = `
        <tmplate>${svgData.svg}</tmplate>
        <script> export default { name: '${name}'};</script>
      `;
      jetpack.write(`tmp/components/${this._library.id}/${name}.vue`, tmplate);
    } else {
      jetpack.write(
        `tmp/svg/${this._library.id}/original/${name}.svg`,
        svgData.original
      );
      jetpack.write(`tmp/svg/${this._library.id}/new/${name}.svg`, svgData.svg);
    }
  }
